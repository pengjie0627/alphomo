<template>
  <div class="sale-edit-grid">
    <qf-table :data="lines" @selection-change="selectionChange" ref="skuTable" row-key="id"
      ...
      <qf-table-column label="商品名称" prop="name" minWidth="160px">
        <template slot-scope="props">
          <search
            :customFlag="true"
            display-field="name"
            :value="getRowSku(props.row.sku)"
            @select="setRowSku($event, props.row, props.rowIndex)"
            :queryParam="skuLineQueryParam"
            @clear="onSkuClear($event, props.row, props.rowIndex)"
            ref="name">
          </search>
          <!--<div style="padding-left: 15px">{{props.row.sku ? props.row.sku.barcode : ''}}</div>-->
        </template>
      </qf-table-column>
      ...
      <qf-table-column label="税额" prop="taxAmount" :formatter="taxAmountFormat" align="right" width="110px;"
...
<script lang="ts" src="./SaleEditTable.ts"></script>
