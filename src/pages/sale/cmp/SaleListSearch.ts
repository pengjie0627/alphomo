...
export default class SaleListSearch extends Vue {
  // 查询数据对象
  queryCondition: SaleQueryCondition = new SaleQueryCondition()
  // 界面长度限制
  ...
  onRouteChange(to: any, from: any) {
    if (from.name !== 'saleView' && from.name !== 'saleList' && to.name === 'saleList') {
      this.doReset()
  ...
  doReset() {
    let filters: FilterParam[] = []
    this.queryCondition = new SaleQueryCondition()
