...
export default class InventoryCheckSearch extends Vue {
  // 查询数据对象
  ...
  onRouteChange(to: any, from: any) {
    if (from.name !== 'inventoryCheckView' && from.name !== 'inventoryCheckList' && to.name === 'inventoryCheckList') {
      this.doReset()
  ...
  doSearch() {
    ...
    if (this.queryCondition.sku) {
      filters.push(new FilterParam('sku:exist', this.queryCondition.sku))
    }
    if (this.queryCondition.manager) {
      filters.push(new FilterParam('manager:=', this.queryCondition.manager.id))
    }
    ...
    if (this.queryCondition.checkDate && this.queryCondition.checkDate.length) {
      filters.push(new FilterParam('checkDate:[,]', this.queryCondition.checkDate))
    }
