  ...
  <qf-table :data="data" @sort-change="doSortChange" @selection-change="doSelectionChange"
    ...
    <qf-table-column label="操作" align="center">
      <template slot-scope="props">
        ...
        <qf-button type="link" v-if="props.row.status === 'UNAUDITED'&&hasPermissions('inventory.deliver.deliver') "
                   @click="doOutInventory(props.row)">出库
        </qf-button>
      </template>
    </qf-table-column>
    <qf-table-column label="业务单号" sortable prop="billNum">
      <template slot-scope="props">
        <qf-button type="link" @click="doGoDetail(props.row.id)" v-if="hasPermissions('inventory.deliver.view')">
          {{props.row.billNum}}
        </qf-button>
        <span v-if="!hasPermissions('inventory.deliver.view')">{{props.row.billNum}}</span>
      </template>
    </qf-table-column>
    ...
    <qf-table-column label="出库仓库" prop="warehouse.name"></qf-table-column>
...
<script lang="ts" src="./InventoryOutTable.ts"></script>
