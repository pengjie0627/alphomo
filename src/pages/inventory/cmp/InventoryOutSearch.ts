...
class InventoryCondition {
  ...
  status: string = '' // 状态
  remark: string = '' // 备注
  thinLogisticsCompanyName: string = ''  // 物流公司
  thinLogisticsNum: string = '' // 物流单号
}
...
export default class InventoryOutSearch extends Vue {
  // 查询数据对象
  ...
  onRouteChange(to: any, from: any) {
    if (from.name !== 'inventoryOutView' && from.name !== 'inventoryOutList' && to.name === 'inventoryOutList') {
      this.doReset()
  ...
  doSearch() {
    ...
    if (this.queryCondition.thinLogisticsCompanyName) {
      filters.push(new FilterParam('thinLogisticsCompanyName:%=%', this.queryCondition.thinLogisticsCompanyName))
    }
    if (this.queryCondition.thinLogisticsNum) {
      filters.push(new FilterParam('thinLogisticsNum:%=%', this.queryCondition.thinLogisticsNum))
    }
