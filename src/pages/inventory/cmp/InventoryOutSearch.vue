<template>
  <query-condition ref="query" @search="doSearch" @reset="doReset" @toggle="doToggle" class="inventory-out-search">
        ...
        <qf-col :span="8">
          <qf-form-item label="状态">
            <qf-select v-model="queryCondition.status" clearable>
              <qf-option value="" label="全部"></qf-option>
              <qf-option value="UNAUDITED" label="未出库"></qf-option>
              <qf-option value="AUDITED" label="已出库"></qf-option>
      ...
      <qf-row>
        <qf-col :span="8">
          <qf-form-item label="物流公司">
            <qf-input placeholder="请输入" v-model="queryCondition.thinLogisticsCompanyName"
                      :maxlength="limits.billNum" @keydown.native.enter="doEnterSearch()"></qf-input>
          </qf-form-item>
        </qf-col>
        <qf-col :span="8">
          <qf-form-item label="物流单号">
            <qf-input placeholder="请输入" v-model="queryCondition.thinLogisticsNum"
                      :maxlength="limits.billNum" @keydown.native.enter="doEnterSearch()"></qf-input>
          </qf-form-item>
        </qf-col>
      </qf-row>
...
<script lang="ts" src="./InventoryOutSearch.ts"></script>

<style lang="scss">
  .inventory-out-search {
