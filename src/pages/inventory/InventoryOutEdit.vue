<template>
  <qf-page-body class="inventory-out-edit" :menu="menu">

    <div slot="actions">
      <qf-checkbox label="自动拆单出库" v-model="canInput" @input="checkStatus"></qf-checkbox>
      <qf-button type="primary" @click="doConfirmOut" v-if="hasPermissions('inventory.deliver.deliver')">确定出库</qf-button>
      <qf-button type="default" @click="doGoback">取消</qf-button>
    </div>

    <qf-bill-body>
      <qf-bill-view-title slot="title" title="出库单">
      ...
      <qf-row slot="remark">
        <qf-col :span="8">
          <qf-form-item label="物流公司：">
            <qf-select v-model="logisticsModel" placeholder="请选择物流公司" clearable>
              <qf-option v-for="(item,index) in thinLogisticsList" :key="index" :value="item"
        ...
        <qf-col :span="8">
          <qf-form-item label="物流单号：">
            <qf-input v-model="logistics.num" :maxlength="64" placeholder="请输入物流单号"></qf-input>
          </qf-form-item>
        </qf-col>
        <qf-col :span="8">
          <qf-form-item label-width="110px" label="预估到货日期：">
            <qf-date-picker placeholder='请选择到货日期' v-model="logistics.estimatedTime"
                            type="date"></qf-date-picker>
          </qf-form-item>
        </qf-col>
      </qf-row>
      <qf-row slot="remark">
      ...
      <div slot="bottomActions">
        <qf-checkbox v-model="canInput" label="自动拆单出库" @input="checkStatus"></qf-checkbox>
        <qf-button type="primary" @click="doConfirmOut" v-if="hasPermissions('inventory.deliver.deliver')">确定出库</qf-button>
        <qf-button type="default" @click="doGoback">取消</qf-button>
      </div>
...
<script lang="ts" src="./InventoryOutEdit.ts"></script>
