  ...
  <qf-dialog-def class="verification-sheet" title="核销单据"
    ...
    <div class="content">
        ...
        <qf-row>
          ...
          <qf-col :span="8">
            <qf-form-item label="经办人" label-width="80px">
              <search display-field="name" v-model="manager" @input="onManagerChange" placeholder="请输入名称查询"
                      type="user"/>
            </qf-form-item>
          </qf-col>
        </qf-row>
      ...
      <qf-table class="sale-list-table" :data="data" @selection-change="doSelectionChange"
        ...
        <qf-table-column label="应付金额(元)" sortable prop="realAmount" :formatter="priceFormatter"></qf-table-column>
        <qf-table-column label="剩余应付(元)" sortable prop="remainedAmount" :formatter="priceFormatter"></qf-table-column>
      </qf-table><!-- 翻页 -->
