...
@Component({
  name: 'PurchaseEditTable',
  components: {
    Search
  }
})
export default class PurchaseEditTable extends Vue {
  ...
  setRowSku(sku: Sku, row: PurchaseLine, rowIndex: number) {
    ...
    if (!item) {
      let line: PurchaseLine = this.buildPurchaseLine(sku)
  ...
  onSkuClear(sku: Sku, row: PurchaseLine, rowIndex: number) {
  ...
  isEmptyLine(line: PurchaseLine): boolean {
  ...
  // 选择单位 更改单价
  munitSelectChange(munit: string, line: PurchaseLine, sku: ThinSku) {
  ...
  doSelectMunitList(row: PurchaseLine) {
  ...
  onTableKeyDown(event: any, rowIndex: number, colIndex: number, row: PurchaseLine, column: any) {
    // 回车事件
    if (event.keyCode === 13) {
  ...
  onRowChange(column: string, row: PurchaseLine) {
    if ('price' === column) {
  ...
  addSkus(skus: Array<Sku>) {
    ...
    skus.forEach(sku => {
      let line: PurchaseLine = this.buildPurchaseLine(sku, false)
      let lastLine: PurchaseLine = this.lines[this.lines.length - 1]
  ...
  addEmptyLine() {
    let empty = new PurchaseLine()
    empty.id = ObjectUtil.uuid()
    this.lines.push(empty)
  }

  removeLines(selected: PurchaseLine[]) {
    if (selected === null || selected.length <= 0) {
      return
    }
    let lines: Array<PurchaseLine> = this.lines
  ...
  freeTaxRate(selected: PurchaseLine[]) {
    if (selected === null || selected.length <= 0) {
      return
    }
    let lines: Array<PurchaseLine> = this.lines
  ...
  buildPurchaseLine(sku: Sku, isAdd: boolean = true): PurchaseLine {
    let line: PurchaseLine = new PurchaseLine()
