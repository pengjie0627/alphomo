<template>
  <!-- 数据列表 -->
  <qf-table class="purchase-list-table" :data="data" @sort-change="doSortChange"
    ...
    <qf-table-column label="操作" width="150px" align="center">
      <template slot-scope="props">
        <detail-button class="purchase-list-button-group" :bill="props.row" target="list"
                       type="link" @getList="getList"></detail-button>
      </template>
    </qf-table-column>
    <qf-table-column label="单号" sortable prop="billNum">
      <template slot-scope="props">
        <div class="table-group">
          ...
          <qf-pill type="primary" size="small"
                   v-if="props.row.purchaseReturns && props.row.purchaseReturns.length">有退货
          </qf-pill>
        </div>
      </template>
    </qf-table-column>
    <qf-table-column label="日期" sortable prop="businessDate"
                     :formatter="dateFormatter"></qf-table-column>
    ...
    <qf-table-column
      ...
      label="金额(去税)" sortable
      prop="taxExcAmount" :formatter="priceFormatter" align="right"></qf-table-column>
    <qf-table-column v-if="hasPermissions('price.refPurchasePrice')" label="金额(含税)" sortable
                     prop="realAmount" :formatter="priceFormatter" align="right"></qf-table-column>
...
<script lang="ts" src="./PurchaseListTable.ts"></script>
<style lang="scss">
