<template>
  <div class="sale-rpt-dtl-water">
    <qf-table
      ...
      <qf-table-column label="单号" prop="billNum" align="left" width="130px"/>
      <qf-table-column label="业务类型" prop="billType" align="left">
        <template slot-scope="props">
          <div>{{props.row.billType === 'Sale' ? '销售' : '销售退货'}}</div>
        </template>
      </qf-table-column>
      <qf-table-column label="来源" prop="billSource" align="left">
        <template slot-scope="props">
          <div v-if="props.row.externalBillSource === null && props.row.externalBillType === null">新建</div>
          <div v-if="props.row.externalBillSource === 'cloud_scm' && props.row.externalBillType === 'Order'">商城</div>
          <div v-if="props.row.externalBillSource === 'cloud_scm' && props.row.externalBillType === 'CarSale'">车销</div>
        </template>
      </qf-table-column>
...
<script lang="ts" src="./SaleRptDtlWater.ts">
