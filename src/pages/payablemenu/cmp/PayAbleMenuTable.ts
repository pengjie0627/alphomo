...
export default class PayAbleMenuTable extends Vue {
  // 定义选择的数组
  selectArray: any = []
  ...
  onBtnAudit(row: any) {
    // todo
    PaymentApi.audit(row.id).then(resp => {
      if (resp.success) {
  ...
  onBtnEdit(row: any) {
    ...
    this.$router.push({ name: 'payAbleMenuAdd',
      query: { type: 'edit', uuid: row.id, ids: JSON.stringify(ids),
  ...
  onBtnAbolish(row: any) {
    PaymentApi.abolish(row.id).then(resp => {
      if (resp.success) {
