...
@Component({
  name: 'SaleReturnEditTable',
  components: {
    Search
  }
})
export default class SaleReturnEditTable extends Vue {
  ...
  setRowSku(sku: Sku, row: SaleReturnLine, rowIndex: number) {
    ...
    if (!item) {
      let line: SaleReturnLine = this.buildSaleReturnLine(sku)
  ...
  onSkuClear(sku: Sku, row: SaleReturnLine, rowIndex: number) {
  ...
  isEmptyLine(line: SaleReturnLine): boolean {
  ...
  // 选择单位 更改单价
  munitSelectChange(munit: string, line: SaleReturnLine, sku: ThinSku) {
  ...
  doSelectMunitList(row: SaleReturnLine) {
  ...
  onTableKeyDown(event: any, rowIndex: number, colIndex: number, row: SaleReturnLine, column: any) {
    // 回车事件
    if (event.keyCode === 13) {
  ...
  onRowChange(column: string, row: SaleReturnLine) {
    if ('price' === column) {
  ...
  addSkus(skus: Array<Sku>) {
    ...
    skus.forEach(sku => {
      let line: SaleReturnLine = this.buildSaleReturnLine(sku, false)
      let lastLine: SaleReturnLine = this.lines[this.lines.length - 1]
  ...
  removeLines(selected: SaleReturnLine[]) {
    if (selected === null || selected.length <= 0) {
      return
    }
    let lines: Array<SaleReturnLine> = this.lines
  ...
  freeTaxRate(selected: SaleReturnLine[]) {
    if (selected === null || selected.length <= 0) {
      return
    }
    let lines: Array<SaleReturnLine> = this.lines
  ...
  buildSaleReturnLine(sku: Sku, isAdd: boolean = true): SaleReturnLine {
    let line: SaleReturnLine = new SaleReturnLine()
