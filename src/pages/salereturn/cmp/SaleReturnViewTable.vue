<template>
  <div class="sale-return-view-table">
    <qf-table :data="bill.lines" row-key="id" showSummary :summary-method="getSummaries">
      ...
      <qf-table-column v-if="merchantConfig.enableOutputTaxRateSupport && !merchantConfig.outputOnlyTax" label="单价(去税)"
                       prop="taxExcPrice" align="right" :formatter="priceFormatter"></qf-table-column>
      <qf-table-column label="单价(含税)" prop="price" align="right" :formatter="priceFormatter"></qf-table-column>
      ...
      <qf-table-column label="税率" prop="taxRate" align="right">
        <template slot-scope="props">
...
<script lang="ts" src="./SaleReturnViewTable.ts"></script>

<style lang="scss">
  @import '~styles/var.scss';

  .sale-return-view-table {
    margin-top: 10px;
    line-height: 36px;
    .summary {
      display: flex;
