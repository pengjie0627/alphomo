...
@Component({
  name: 'SaleReturnViewTable',
  components: {}
})
export default class SaleReturnViewTable extends Vue {
  ...
  mounted() {
    this.realAmount = Number(this.bill!.amount) + Number(this.bill!.taxAmount) + Number(this.bill!.chargeAmount) - Number(this.bill!.discountAmount)
  }
...
SaleReturnViewTable.filter('price', (value: string) => {
  if (value && Number(value) !== 0) {
    return Number(value).toFixed(2)
  } else {
    return '0.00'
